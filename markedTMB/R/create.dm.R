...
#'              chunk_size=1e7,remove.unused.columns=TRUE,simplify=FALSE)
#' 
...
#' for each parameter with a sub-list for the fixed effect (fe) and random effects. re.list is a list with an element for each random component containing re.dm and indices for
...
create.dml=function(ddl,model.parameters,design.parameters,restrict=FALSE,chunk_size=1e7,remove.unused.columns=TRUE,simplify=FALSE)
{

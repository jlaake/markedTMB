...
msjsu_tmb=function(x,ddl,fullddl,dml,model_data=NULL,parameters,accumulate=TRUE,initial=NULL,method,
                   ...
                   re=FALSE,extra.args="",getreals=FALSE, useHess=FALSE,savef=TRUE, ...)
{
  accumulate=FALSE
  nocc=x$nocc
  ...
  tmb_data <- list(nG=nG,n=length(model_data$imat$freq),m=model_data$imat$nocc,nS=length(strata.labels),
  ...
  tmb_par <- list(phibeta=par$S,pbeta=par$p,psibeta=par$Psi,pentbeta=par$pent,pibeta=par$pi,deltabeta=par$delta,log_sigma_phi=rep(-1,phi_relist$nsigma),
                  ...
                  u_p=rep(0,p_relist$nre),u_psi=rep(0,psi_relist$nre),u_pent=rep(0,pent_relist$nre),u_pi=rep(0,pi_relist$nre),u_delta=rep(0,delta_relist$nre))
  f = MakeADFun(
    data=c(model="msjsu",tmb_data),
    parameters=tmb_par,
    random=c("u_phi","u_p","u_psi","u_pent","u_pi","u_delta"),

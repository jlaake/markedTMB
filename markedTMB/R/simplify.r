...
simplify_ddl=function(ddl,parameters)
{
	for (parname in names(parameters))
	{
		...
		if(!is.null(ddl[[parname]]$time.interval)) fields=c(fields,"time.interval")
		if(length(fields)==0)
...
{
  nre_byeffect=apply(mixed$re.indices,2,function(x) max(x)-min(x)+1)
...
return(list(nsigma=nsigma,nre=nre,nre_byeffect=nre_byeffect,krand=krand,counts=counts,randDM=randDM,randDM_i=randDM_i,randIndex=randIndex,randIndex_i=randIndex_i,idIndex=idIndex,idIndex_i=idIndex_i))
}

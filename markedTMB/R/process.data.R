...
initial.ages=c(0),time.intervals=NULL,nocc=NULL,accumulate=TRUE,strata.labels=NULL)
{
  ...
  #
   ...
   #  if strata.labels then the first non-zero x is matched to strata.labels to exclude possible "U" values
   ...
   # } else {
    chsplt=","
    if(model=="MSLD")chsplt=""
    if(is.null(model.list$hmm$strata.labels))
          start=t(sapply(data$ch,function(x){
				   	xx=strsplit(x,chsplt)[[1]]
					  ich=min(which(strsplit(x,chsplt)[[1]]!="0"))

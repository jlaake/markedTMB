                   ...
                   re=FALSE,compile=FALSE,extra.args="",clean=FALSE,getreals=FALSE, useHess=FALSE,savef=FALSE,...)
{
  ...
  fullddl=NULL
  load("tmp.rda")
  first_rec=sapply(split(1:nrow(fullddl$S),fullddl$S$id),function(x)x[1])
  first_rec_Psi=sapply(split(1:nrow(fullddl$Psi),fullddl$Psi$id),function(x)x[1])
  accumulate=FALSE
  nocc=x$nocc
  ...
  f = MakeADFun(data=list(n=length(model_data$imat$freq),m=model_data$imat$nocc,nS=length(strata.labels)-1,
                          ch=chmat,frst=model_data$imat$first,frstrec=first_rec,frstrecpsi=first_rec_Psi,
                          freq=model_data$imat$freq,tint=model_data$time.intervals,

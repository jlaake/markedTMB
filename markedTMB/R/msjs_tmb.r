                  ...
                  re=FALSE,extra.args="",getreals=FALSE, useHess=FALSE,savef=TRUE, ...)
{
  ...
  tmb_par <- list(phibeta=par$S,pbeta=par$p,psibeta=par$Psi,pentbeta=par$pent,pibeta=par$pi,log_sigma_phi=rep(-1,phi_relist$nsigma),
                  ...
                  u_p=rep(0,p_relist$nre),u_psi=rep(0,psi_relist$nre),u_pent=rep(0,pent_relist$nre),u_pi=rep(0,pi_relist$nre))
  
  f = MakeADFun(
    data=c(model="msjs",tmb_data),
    parameters=tmb_par,
    random=c("u_phi","u_p","u_psi","u_pent","u_pi"),

                   ...
                   re=FALSE,extra.args="",getreals=FALSE, useHess=FALSE,savef=TRUE, ...)
{
  ...
  tmb_data <- list(n=length(model_data$imat$freq),m=model_data$imat$nocc,nS=length(strata.labels),
  ...
  tmb_par <- list(phibeta=par$S,pbeta=par$p,psibeta=par$Psi,log_sigma_phi=rep(-1,phi_relist$nsigma),

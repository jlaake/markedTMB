...
template<class Type>
Type mvms(objective_function<Type>* obj) {
  ...
  array<double> alldmat(n,m,nobs,nS+1);      // observation probability matrices  for all individuals
  array<double> alldelta(n,nS);      // initial state distribution for all individuals; related to pi and not delta parameter
        ...
        for(j=frst(i);j<=m;j++)
        {
          ...
          for(k=0;k<=npos-1;k++)
          {
            ...
            if(j==frst(i))
            {
              ...
              alldmat(i,j-1,irow-1,icol-1)=asDouble(dmat(j-1,irow-1,icol-1));
              dmat(j-1,0,icol-1)=0;
              alldmat(i,j-1,0,icol-1)=0;
            } else
            {
              dmat(j-1,irow-1,icol-1)=p(bindex+icol-1)*delta(j-1,k);
              dmat(j-1,0,icol-1)=1-p(bindex+icol-1);
              alldmat(i,j-1,irow-1,icol-1)=asDouble(dmat(j-1,irow-1,icol-1));
              alldmat(i,j-1,0,icol-1)=asDouble(dmat(j-1,0,icol-1));
            }
          }
          dmat(j-1,0,nS)=1;
          alldmat(i,j-1,0,nS)=1;
        }

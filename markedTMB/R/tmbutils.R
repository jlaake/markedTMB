...
{  	
  if(model=="MSCJS")
    if(!"multistate_tmb"%in%names(getLoadedDLLs()))
      dyn.load(dynlib("multistate_tmb"))
  if(model=="MVMSCJS")
    if(!"mvms_tmb"%in%names(getLoadedDLLs()))
      dyn.load(dynlib("mvms_tmb"))
  if(model=="MSLD")
    if(!"msld_tmb"%in%names(getLoadedDLLs()))
      dyn.load(dynlib("msld_tmb"))
  return(NULL)
}
